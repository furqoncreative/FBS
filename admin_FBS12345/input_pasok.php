...
if(isset($_POST['tambahpasok'])) {

    if(tambahpasok($_POST)> 0 ) {

        echo "<script>
             alert('DATA PASOK BERHASIL DITAMBAH !');
             document.location.href='index.php';
             </script>"; 
         }
        else {
            echo "<script>
             alert('DATA PASOK GAGAL DITAMBAH !');
             document.location.href='input_pasok.php';
             </script>";
        }
    }
?>
...
//koneksi berhsil
 ...
 $carikode = mysqli_query($koneksi, "SELECT max(id_pasok) from pasok") or die (mysql_error());
  // menjadikannya array
  ...
  if ($datakode) {
   ...
   $kode = $kode + 1;
   $kode_otomatis = "PSK-".str_pad($kode, 3, "0", STR_PAD_LEFT);
  } else {
   $kode_otomatis = "PSK-001";
  }
...
<div class="registration">
		...
		<h2><span><i class="fa fa-pencil"></i> Input Data Pasok Baru</span></h2>
		<script>
			(function() {
		 ...
		 <!-- Form -->
			<form method="POST" action="">
				<div>
					<label>
						<input placeholder="ID Pasok :" type="text" name="id_pasok" value="<?php echo $kode_otomatis;?>" readonly>
					</label>
				</div>
				<div>
					<select class="form-control1"  id="id_buku" name="id_distributor" >
					<option>- ID Distributor -</option>
					<?php
      				$sql=query("SELECT * FROM Distributor");
     				 foreach($sql as $data):
 					 ?>
					<option value="<?php echo $data['id_distributor'];?>"><?php echo $data['id_distributor'];?></option>
					<?php endforeach; ?>
					</select>
				</div>							
				<div>
					<input type="submit" name="tambahpasok" tabindex="5" value="SUBMIT">
					<input type="reset" tabindex="6" value="RESET">
				</div>
			<!-- /Form -->
		</div>
	</div>
	<div class="registration_left">
		<h2> .</h2>
		 ...
		 <!-- Form -->
				...
				<div>
					<label>
						<input placeholder="Jumlah :" type="text" name="jumlah" tabindex="3" required="">
					</label>
				</div>	
				<div>
				<?php
				...
				?>
					<label >
						<input placeholder="Tanggal :" type="text" name="tanggal" tabindex="4" required="" value="<?php echo $tgl;?>" readonly>
					</label>
				</div>					
			</form>
			<!-- /Form -->
